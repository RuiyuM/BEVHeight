# active_learning/methods/binEnt30_twoTier_global_then_classanti_adapt_stage23.py
# Stage-23：仅保留 Stage-B + Stage-C（删除 Stage-A）
...
# =============================================================
# Stage-B + Stage-C（Stage-A 删除）
# B = 两层 diversity（B2=log-ratio 贪心）
# C = 简化：C1=Global NLL“最相似”；C2=按类等权配额“最不相似”贪心
# =============================================================
class BinEnt30_TwoTier_Stage23_GlobalThenClassAntiAdaptiveSelector(BaseActiveSelector):
    def __init__(
        self,
        # ------- Stage-A 参数（本变体不使用，保留仅为兼容配置） -------
        ...
        **kwargs
        ...
        # 注意：Stage-A 被删除，因此不对 A 的参数做约束（避免你把 A 设成 0 时触发 assert）
    ...
    def _decode_batch(self, model, sweep_imgs, mats, img_metas, device,
        ...
        # 重要：尽量兼容两类 backbone：
        # 1) 支持 return_depth_profile / depth_profile_* 参数
        # 2) 旧版本不支持深度覆盖参数
        try:
            ...
            if need_depth_profile:
                raise RuntimeError(
                    "Backbone 不支持 return_depth_profile/depth_profile_* 参数，但本变体需要 depth profile。"
                ) from e
            out = model.model.backbone(
                sweep_imgs, mats,
                is_return_height=False,
    ...
    def select(
        ...
        # ---- 扫描未标注池：H_img + 预测（Stage-A 删除，因此不需要 depth profile） ----
        recs = self._collect_pool_records(
            model, dataloader, device, unlabeled_indices,
            need_depth_profile=False,
        )

        # ===================== Stage-B（从全池开始） =====================
        cand_A = recs  # Stage-A 删除：直接把全池当作 B 的输入

        # ---------- B1) per-image diversity gate ----------
        ...
        # C2：按类等权配额，贪心挑“最不相似”（per-class NLL 最大）
        K2 = max(0, k - K1)
        if K2 > 0:
...
try:
    ...
    _METHOD_REGISTRY["binEnt30_twoTier_global_then_classanti_adapt_stage23"] = BinEnt30_TwoTier_Stage23_GlobalThenClassAntiAdaptiveSelector
except Exception:
    pass
