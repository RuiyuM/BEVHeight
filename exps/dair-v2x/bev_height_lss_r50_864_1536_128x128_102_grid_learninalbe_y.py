...
img_conf = dict(img_mean=[123.675, 116.28, 103.53],
                ...
                to_rgb=True)
def load_ckpt_with_adapter(model, ckpt_path):
    ckpt = torch.load(ckpt_path, map_location='cpu')
    sd = ckpt['state_dict']

    # 1) 从 ckpt 里推断 adapter 的形状（[out, in, 1, 1]）
    w_key = 'model.backbone.adapter.weight'
    if w_key in sd:
        out_ch, in_ch, _, _ = sd[w_key].shape

        # 2) 若当前模型还没有 adapter，就按 ckpt 的形状先造一个
        bkb = model.model.backbone  # 你的 BEVHeight 里的 backbone
        if not hasattr(bkb, 'adapter') or bkb.adapter is None:
            bkb.adapter = nn.Conv2d(in_ch, out_ch, kernel_size=1, bias=False)

    # 3) 再加载权重（这时 keys 对得上了）
...
class BEVHeightLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        # —— 可学习 y 的正则系数（可按需调）——
        self.lambda_y_smooth = 1e-4  # 平滑项系数
        self.lambda_y_shape = 1e-3  # 形变幅度项系数
        self.freeze_y_epochs = 5  # 例如先冻结前 0~5 个 epoch 可设为 5

    def _get_y_mapping_reg(self):
        """遍历子模块，累加 y 映射正则（如果存在）"""
        device = next(self.model.parameters()).device
        reg = torch.tensor(0.0, device=device)
        for m in self.model.modules():
            if hasattr(m, 'y_mapping_regularization'):
                reg = reg + m.y_mapping_regularization(
                    lambda_smooth=self.lambda_y_smooth,
                    lambda_shape=self.lambda_y_shape
                )
        return reg
    ...
    def training_step(self, batch):
        ...
        # —— y 映射正则（可选冻结前若干个 epoch）——
        if self.current_epoch < self.freeze_y_epochs:
            reg_y = torch.tensor(0.0, device=detection_loss.device)
        else:
            reg_y = self._get_y_mapping_reg()

        loss = detection_loss + reg_y

        # 记录
        self.log('loss', loss, prog_bar=True)
        self.log('detection_loss', detection_loss, prog_bar=True)
        self.log('reg_y', reg_y, prog_bar=True)
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='/data/rxm210041/outputs/bev_height_lss_r50_864_1536_128x128_grid_learninalbe_y/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    ...
    if args.evaluate:
        ...
        def _test_one(path_str):
            ...
            mdl = BEVHeightLightningModel(**vars(args))
            mdl = load_ckpt_with_adapter(mdl, path_str)
            trainer.test(mdl, ckpt_path=None)
        ...
        backup_codebase(
            os.path.join('/data/rxm210041/outputs/bev_height_lss_r50_864_1536_128x128_grid_learninalbe_y', 'backup'))
        trainer.fit(model)
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=32,
        default_root_dir='/data/rxm210041/outputs/bev_height_lss_r50_864_1536_128x128_grid_learninalbe_y')
