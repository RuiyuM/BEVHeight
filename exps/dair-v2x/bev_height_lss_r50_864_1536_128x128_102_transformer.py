...
custom_imports = dict(imports=['vit_backbone'], allow_failed_imports=False)
...
backbone_conf = {
    ...
    'z_bound':         [-5, 3, 8],
    'd_bound':         [-2.0, 0.0, 90],
    'final_dim':       final_dim,          # e.g. (256, 704)
    'output_channels': 80,
    'downsample_factor': 16,

    # ---------- ① ViT 主干 ----------
    'img_backbone_conf': dict(
        type='ViTBackbone',          # ← 自定义注册的类
        arch='tiny',                 # tiny / small / base / large
        img_size=final_dim,          # 与训练输入 crop 尺寸一致
        patch_size=16,
        out_indices=(2, 5, 8, 11),   # 4 个层级
        drop_path_rate=0.1,
        init_cfg=dict(
            type='Pretrained',
            checkpoint='https://download.pytorch.org/models/vit_b_16-c867db91.pth'
        )
    ),
    ...
    'img_neck_conf': dict(
        type='SECONDFPN',
        in_channels=[192, 192, 192, 192],   # ViT-Base 每层都是 768
        upsample_strides=[1, 1, 1, 1],
        out_channels=[128, 128, 128, 128]   # 保持与旧版一致
    ),

    # ---------- ③ HeightNet ----------
    'height_net_conf': dict(
        in_channels=512,  # FPN 4 个输出 concat 后 128×4 = 512
        mid_channels=512
    )
}
ida_aug_conf = {
