...
# --- Torch1.9 兼容补丁（safetensors & SDPA），以及 vggt 源码路径 ---
import sys, math, numpy as np, torch, torch.nn.functional as F
sys.path.insert(0, "/people/cs/r/rxm210041/Desktop/test_3d_active/vggt")  # 没配 .pth 的话必须要
import inspect
import torch.utils.checkpoint as tucp

# ========= Torch1.9 兼容补丁（必须在 import vggt 之前执行）=========
...
if not hasattr(torch, "frombuffer"):
    _TORCH2NP = {
        torch.uint8: np.uint8, torch.int8: np.int8, torch.int16: np.int16,
        torch.int32: np.int32, torch.int64: np.int64,
        torch.float16: np.float16, torch.float32: np.float32, torch.float64: np.float64,
        torch.bool: np.bool_
    }
    def _frombuffer(buffer, dtype=torch.uint8, count=-1, offset=0, *, like=None, device=None):
        mv = memoryview(buffer)
        if offset: mv = mv[offset:]
        np_dtype = _TORCH2NP.get(dtype, np.uint8) if isinstance(dtype, torch.dtype) else dtype
        arr = np.frombuffer(mv, dtype=np_dtype, count=(count if count!=-1 else -1))
        if not arr.flags.writeable: arr = np.copy(arr)
        return torch.from_numpy(arr)
    torch.frombuffer = _frombuffer

# 2) scaled_dot_product_attention
if not hasattr(F, "scaled_dot_product_attention"):
    def _sdp_attention(q, k, v, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None):
        d = q.size(-1)
        s = torch.matmul(q, k.transpose(-2,-1)) * (scale if scale is not None else (1.0/math.sqrt(d)))
        if attn_mask is not None:
            if attn_mask.dtype == torch.bool: s = s.masked_fill(~attn_mask, float('-inf'))
            else: s = s + attn_mask
        if is_causal:
            Lq, Lk = q.size(-2), k.size(-2)
            causal = torch.triu(torch.ones((Lq,Lk), dtype=torch.bool, device=q.device), diagonal=1)
            s = s.masked_fill(causal, float('-inf'))
        a = torch.softmax(s, dim=-1)
        if dropout_p and dropout_p>0: a = F.dropout(a, p=dropout_p, training=True)
        return torch.matmul(a, v)
    F.scaled_dot_product_attention = _sdp_attention

# 3) interpolate: 忽略 antialias，且 bicubic+fp16 时临时升精度
if "antialias" not in F.interpolate.__code__.co_varnames:
    _orig_interpolate = F.interpolate
    def _interp_compat(input, size=None, scale_factor=None, mode="nearest",
                       align_corners=None, recompute_scale_factor=None, **kwargs):
        kwargs.pop("antialias", None)
        need_cast = (mode == "bicubic") and (input.dtype in (torch.float16, torch.bfloat16))
        if need_cast:
            out = _orig_interpolate(input.float(), size=size, scale_factor=scale_factor,
        ...
        return _orig_interpolate(input, size=size, scale_factor=scale_factor,
                                 mode=mode, align_corners=align_corners,
                                 recompute_scale_factor=recompute_scale_factor, **kwargs)
    F.interpolate = _interp_compat

# 4) checkpoint: 无条件剥掉 use_reentrant
_ckpt_orig = _tucp.checkpoint
def _ckpt_no_reentrant(function, *args, **kwargs):
    kwargs.pop("use_reentrant", None)
    return _ckpt_orig(function, *args, **kwargs)
_tucp.checkpoint = _ckpt_no_reentrant
# ================================================================
import vggt_backbone
...
custom_imports = dict(imports=['vggt_backbone'], allow_failed_imports=False)
...
backbone_conf = {
    ...
    # ① VGGT 主干（内部自动 pad 到 14 倍数 + 4层金字塔 + 1x1投影为256）
    'img_backbone_conf': dict(
        ...
        freeze_vggt=True,
        use_amp=True,
        # 下面三项用于把 DataLoader 的 mmcv 归一化撤销回 0~1
        img_mean=img_conf['img_mean'],
        img_std=img_conf['img_std'],
        to_rgb=img_conf['to_rgb'],
    ),

    # ② FPN 颈部：把 in_channels 改为 [256,256,256,256]
    'img_neck_conf': dict(
        type='SECONDFPN',
