...
class LSSFPN(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, final_dim,
        ...
        self._init_bev_grid(x_bound, y_bound, nonuniform_beta=None)  # 或者给个 beta=2.0 试远密近疏
        self.adapter = None
    ...
    def _init_bev_grid(self, x_bound, y_bound, nonuniform_beta=None):
        ...
        if nonuniform_beta is None:
            ys = torch.linspace(ymin + dy / 2, ymax - dy / 2, ny)
        else:
            # 可选：对数密度（远密近疏）
            t = torch.linspace(0, 1, ny)
            w = (torch.exp(nonuniform_beta * t) - 1.0) / (torch.exp(nonuniform_beta) - 1.0)
            ys = ymin + w * (ymax - ymin)

        yy, xx = torch.meshgrid(ys, xs)  # (HB,WB)
        XY1 = torch.stack([xx, yy, torch.ones_like(xx)], -1)  # (HB,WB,3) [X,Y,1]

        self.HB, self.WB = ny, nx
        self.register_buffer('bev_xy1', XY1)  # 物理坐标（单位：米）
    ...
    def _compute_pixel_grid(self, sensor2ego, intrin, ida, img_hw, feat_hw):
        ...
        # BEV 网格 (X,Y,1)
        XY1 = self.bev_xy1.view(1, 1, HB, WB, 3).to(device)  # (1,1,HB,WB,3)
        X = XY1[..., 0];
        Y = XY1[..., 1]
